...
async function loadStatus() {
  ...
  changeEl.innerHTML = `<span class="change-value">Today ${changeSign}$${Math.abs(change).toFixed(2)}</span> <span class="change-pct">(${changePercent.toFixed(2)}%)</span>`;
  ...
  document.getElementById('kpiWinRate').textContent = data.metrics?.win_rate ? `${(data.metrics.win_rate * 100).toFixed(1)}%` : '--';
  ...
  const tradesHtml = trades.length
    ? trades.map(t => {
        ...
        return `<div class="trade-row"><span>${t.time}</span><span>${t.instrument} ${t.direction}</span><span class="pnl-pill ${pnlClass}">${pnlSign}$${Math.abs(t.pnl).toFixed(2)}</span></div>`;
      }).join('')
...
async function loadHistory() {
  ...
  if (!data || !data.trades || data.trades.length === 0) {
    list.innerHTML = '<div class="trade-row placeholder">No trades yet.</div>';
    return;
  }
  list.innerHTML = data.trades.map(t => {
    const pnlClass = t.pnl >= 0 ? 'positive' : 'negative';
    const pnlSign = t.pnl >= 0 ? '+' : '';
    return `<div class="trade-row"><span>${t.timestamp}</span><span>${t.instrument} ${t.direction}</span><span>Entry ${t.entry} → Exit ${t.exit}</span><span class="pnl-pill ${pnlClass}">${pnlSign}$${Math.abs(t.pnl).toFixed(2)}</span></div>`;
  }).join('');
}
...
async function subscribeToPush() {
  try {
    const vapidResp = await fetch(`${API_URL}/vapid-public-key`, { headers: { Authorization: `Bearer ${API_KEY}` } });
...
async function triggerEmergencyOptimization() {
  ...
  if (res && res.success) {
    alert('Emergency optimization started.');
  } else {
    alert('Failed to start optimization. Server may be down.');
    btn.disabled = false;
  }
}
